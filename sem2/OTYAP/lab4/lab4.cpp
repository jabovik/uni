...
namespace Syntaxical
{
    ...
    void create_table(EState table[LEX_TYPES_NUM][STATES_NUM][LEX_TYPES_NUM]) // лексема(сигнал), текущее состояние, стек
    {
        table[DO][S_INIT][];
    }

    void SynAnalysis(Lex *lex_list)
    {
        EState state = S_INIT;
        while (lex_list)
        {
...
int main(int argc, char const *argv[])
{
    ...
    if (lex_list)
    {
        ...
        output << std::endl;
        Lexical::print_list(lex_list, output, ID);
        output << std::endl;
        Lexical::print_list(lex_list, output, VL);
        output.close();
    }
    delete[] text;
